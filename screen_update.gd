...
func build(body: VBoxContainer) -> void:
	...
	_version = _ligne(body, 14, MenuTheme.DIM)
	_titre = _ligne(body, 20, MenuTheme.P1)
	_detail = _ligne(body, 14, MenuTheme.DIM)
	_notes = _ligne(body, 14, Color(0.78, 0.8, 0.85))
	...
	# n'a rien demandé.
	_avis = _ligne(body, 14, MenuTheme.WARN)
	_empeche = _ligne(body, 13, MenuTheme.WARN)
