...
| Décision | Raison |
|---|---|
| **Refonte des mécaniques de tir : munitions finies, dispersion bloom et rechargement** (2026-09-07, Adrien) | Chaque arme possède un chargeur fini, une cadence propre, une dispersion dynamique au tir enchaîné et un temps de recharge distinct : Pistolet (10 munitions, cooldown 0.16s, recharge 1.1s, bloom +4.5°/tir max 25°), Fusil (24 munitions, cooldown 0.24s, recharge 1.7s, bloom +3.5°/tir max 20°), Arbalète (1 munition, cooldown 0.3s, recharge 2.2s auto après tir), Pompe (6 munitions, cooldown 0.9s, recharge 2.8s). Hiérarchie des temps de recharge : Pompe (2.8s) > Arbalète (2.2s) > Fusil (1.7s) > Pistolet (1.1s). Touche de recharge dédiée : Carré (`JOY_BUTTON_X`) sur manette (fusée déplacée sur Triangle `JOY_BUTTON_Y`), R (J1) / K (J2) sur clavier. `Protocol.VERSION` passe à 8 pour transporter l'action de recharge. |
