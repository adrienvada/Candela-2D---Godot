...
- L'encre de la pâte sur les objets debout et le leurre : poser `pate_encre_boite` dans le matériau des `VoxelObjet` (`miroirs_iso.gd` / `capteur_objet.gdshader`), comme ISO Corps l'a fait pour les corps ; noir absolu et équité inchangés (la suite le prouve).
